...
def gradient_descent(oracle, x_0, tolerance=1e-5, max_iter=10000,
    ...
    while np.linalg.norm(oracle.grad(x_k))**2 <= tolerance * np.linalg.norm(oracle.grad(x_0))**2:
        ...
        alpha_k = line_search_tool.line_search(line_search_options)
        x_k -= alpha_k * oracle.grad(x_k)

        if x_k == None or x_k == float('inf'):
            return x_k, 'computational_error', history
        history['time'].append((datetime.now() - start).seconds)
        history['func'].append(oracle.func(x_k))
        history['grad_norm'].append(np.linalg.norm(oracle.grad(x_k)))
        if len(x_k) <= 2:
            history['x'].append(x-K)
        iter += 1
...
def newton(oracle, x_0, tolerance=1e-5, max_iter=100,
    ...
    while np.linalg.norm(oracle.grad(x_k)) ** 2 <= tolerance * np.linalg.norm(oracle.grad(x_0)) ** 2:
        ...
        try:
            c, low = cho_factor(oracle.hess(x_k))
        except LinAlgError:
